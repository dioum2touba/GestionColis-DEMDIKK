...
import CreerDetailslivraison from './creer-detailslivraison';

class CreerLivraion extends React.Component {
  ...
  render() {
    const colisJSX = this.state.loading? 
        this.state.repos.map((elt) => {
          ...
          {
            return (
              <CreerDetailslivraison key={elt.id} creerDetailslivraison={elt} { ...this.props } />
            )
          } else { return null; }
        }) : null;
